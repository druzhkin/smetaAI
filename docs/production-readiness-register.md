...
| No unregistered manual changes | immutable/revisioned observations, passport, quantities, contract, risk and actuals plus audited approval decisions; governed UI supports exact-manifest four-eyes document-set confirmation, source-level conflict resolution, and policy-bound quantity revision with immutable before/after state, exact source evidence, independent approval, author-only server-side application and PostgreSQL immutability guards; generic decisions cannot bypass dedicated conflict resolution, while stale context, reused changes and broken approval linkage fail closed; other controlled mutation surfaces and complete UI/process qualification are absent | **PARTIAL** |
| Dangerous case blocking | negative tests cover new-revision invalidation, stale/tampered snapshots, controlled-version drift, conflict resolution, evidence independence, stale attestations, scope/quantity, policy-versioned price-normalization replay, tampered normalized amounts, commercially incomplete price exclusion/RFQ, contract, risk, component coverage, invented sources, arithmetic and release policy; golden historical set absent | **PARTIAL** |
...
| Operator and reviewer UI | production-built OIDC PKCE interface exposes role-filtered portfolio, work queue, project hard stops, indicators, record lineage and audit history; controlled project registration, quarantined upload/receipt polling, exact-manifest document-set confirmation, source-level conflict resolution, quantity revision and expert decisions use explicit attestations and stable idempotency; quantity revision binds the exact current quantity, confirmed document set, approved policy/rules, immutable before/after state and source observations, requires policy-assigned four-eyes review for critical changes, prevents the reviewer from applying, and lets only the author apply the server-held after-state; conflict resolution and decisions retain their dedicated version/evidence/four-eyes guards; upload UI preserves quarantine/evidence separation and server limits; same-origin navigation, exact decimal rendering, security headers, clean-console desktop/mobile browser QA, frontend tests and container build are implemented; general extraction correction, BoQ structure/nomenclature, pricing, calculation, reconciliation and release actions, accessibility evidence, role-based UAT and business qualification remain absent | **PARTIAL** |
