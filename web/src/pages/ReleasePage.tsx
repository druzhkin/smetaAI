      ...
      {mutation.isSuccess && (
        <section className="success-panel" role="status">
          ...
          <div>
            ...
            <p>
              Проект переведён в{" "}
              <strong>{mutation.data.project.state}</strong>. Решение и переход
      ...
      {decision.allowed && eligible && hasApproverRole ? (
        <form className="entry-form release-form" onSubmit={submit}>
          <div className="entry-form__intro">
            ...
            <p>
              Повторное использование формы после изменения документов,
              расчёта, согласований или проекта будет отклонено сервером по
              несовпадению gate hash.
            </p>
          </div>
          <label>
            Основание решения
